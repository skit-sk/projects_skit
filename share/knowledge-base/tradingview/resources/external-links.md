# External Links

## Официальная документация

### Lightweight Charts
| Ресурс | URL |
|--------|-----|
| Documentation | https://tradingview.github.io/lightweight-charts/ |
| API Reference | https://tradingview.github.io/lightweight-charts/docs/api |
| GitHub | https://github.com/tradingview/lightweight-charts |
| npm | https://www.npmjs.com/package/lightweight-charts |
| Plugin Examples | https://github.com/tradingview/lightweight-charts/tree/master/plugin-examples |

### Advanced Charts
| Ресурс | URL |
|--------|-----|
| Documentation | https://www.tradingview.com/charting-library-docs/ |
| Demo | https://charting-library.tradingview-widget.com/ |

### TradingView Widgets
| Ресурс | URL |
|--------|-----|
| Widget Catalog | https://www.tradingview.com/widget/ |
| Widget Docs | https://www.tradingview.com/widget-docs/ |
| Widget Demos | https://www.tradingview.com/widget-docs/widgets/ |

### Trading Platform
| Ресурс | URL |
|--------|-----|
| Demo | https://trading-terminal.tradingview-widget.com/ |

## Tutorials

| Название | URL |
|---------|-----|
| Lightweight Charts Tutorials | https://tradingview.github.io/lightweight-charts/tutorials/ |
| Widgets Tutorials | https://www.tradingview.com/widget-docs/tutorials/ |
| Iframe Tutorial | https://www.tradingview.com/widget-docs/tutorials/iframe/build-page/ |

## Markets & Data

| Название | URL |
|---------|-----|
| Available Markets | https://www.tradingview.com/widget-docs/markets/ |
| Data Coverage | https://www.tradingview.com/data-coverage/ |

## Community

| Название | URL |
|---------|-----|
| Stack Overflow | https://stackoverflow.com/questions/tagged/lightweight-charts |
| Twitter | https://twitter.com/tradingview |
| GitHub Discussions | https://github.com/tradingview/lightweight-charts/discussions |

## Bug Bounty

| Название | URL |
|---------|-----|
| Bug Bounty Program | https://www.tradingview.com/bounty/ |
| Security | https://www.tradingview.com/security/ |

## Демо в этом проекте

| Раздел | Путь |
|-------|------|
| Lightweight Charts Demos | `/home/user_aioc/workspace/projects/04_tradingview-demos/` |
| Widget Demos | `/home/user_aioc/workspace/projects/04_tradingview-demos/widgets/` |

## Связанные проекты

| Проект | Описание |
|--------|----------|
| ccxt | Криптовалютный API (используется в graphs_candle) |
| Plotly | Библиотека для графиков (используется в graphs_candle) |
