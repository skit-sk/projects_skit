...
PATTERN = re.compile(r"wg\s+([A-Z]+(?::[A-Z]+)?)(?:\s+(\w+))?(?:\s+(\w+))?", re.IGNORECASE)
...
async def take_screenshot(symbol, timeframe, output_dir, range_val=""):
    ...
    url = (
        ...
        f"&studies=%5B%22Volume%40tv-basicstudies%22%5D"
    )
    if range_val:
        url += f"&range={RANGE_MAP.get(range_val, range_val)}"

    log.info(f"Widget screenshot (cloak): {symbol} {timeframe} range={range_val or 'default'}")
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if status == "ok":
                break
            log.warning(f"Widget widget error (attempt {attempt+1}), retrying...")
            await asyncio.sleep(2)
        except Exception as e:
            log.error(f"CloakBrowser widget error (attempt {attempt+1}): {e}")
            if browser:
                try:
                    await browser.close()
                except Exception:
    ...
    log.info(f"Widget screenshot saved: {output_path} ({sz} bytes)")
    if sz < 5000:
