...
Tries to fetch auto-generated subtitles for given YouTube URL and
...
def extract_video_id(url: str) -> str:
    # simple extractor for common variants
...
def fetch_subtitles(url: str, lang_list):
    video_id = extract_video_id(url) if extract_video_id(url) else None
    tmpdir = "/tmp/yttranscripts"
    ensure_dir(tmpdir)
    # Try to download auto subtitles for each language in order
    for lang in lang_list:
        out_template = os.path.join(tmpdir, f"{video_id or 'video'}.{lang}.vtt")
        cmd = ["yt-dlp",
               "--skip-download",
               "--write-auto-sub",
               f"--sub-lang={lang}",
...
def vtt_to_text(path: str) -> str:
    ...
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if "-->" in line:
                continue
            # skip numeric sequence lines often present in transcripts
...
def main():
    ...
    if not sub_path:
        print("No transcript available", file=sys.stdout)
        sys.exit(0)
